...
const fetchApplicantData = async (id) => {
  ...
  return data.data.find((candidate) => candidate.id === id);
};
...
const ApplicantDetails = () => {
  ...
  useEffect(() => {
    const loadApplicantData = async () => {
      try {
        setIsLoading(true);
        const candidateData = await fetchApplicantData(id);

        if (!candidateData) {
          setError(new Error("Candidate not found"));
          return;
        }
  ...
  const passedTests =
    codingSubmission?.testResults?.filter((tr) => tr.passed).length || 0;
              ...
              <div className="info-item">
                ...
                <span
                  className={`info-value ${
                    testAttempt.is_submitted ? "submitted" : "not-submitted"
                  }`}
                >
        ...
        <div className="info-section">
          ...
          <div className="performance-grid">
            ...
            <div className="performance-card">
              ...
              <div
                className={`performance-value ${
                  isSelected ? "score-pass" : "score-fail"
                }`}
              >
                {isSelected ? "SELECTED" : "NOT SELECTED"}
              </div>
              ...
              <div className="coding-header">
                <div className="coding-info">
                  <span className="coding-problem">
                    Problem: {codingSubmission.problemKey}
                  </span>
                  <span
                ...
                <div className="coding-date">
                  Submitted: {formatDate(codingSubmission.createdAt)}
                </div>
              </div>

              <div className="code-section">
                <h4>Code Submitted:</h4>
                <pre className="code-block">{codingSubmission.code}</pre>
              </div>

              {codingSubmission.output && (
                <div className="output-section">
                  <h4>Output:</h4>
                  <pre className="output-block">{codingSubmission.output}</pre>
                </div>
              )}
                  ...
                  <div className="test-results-section">
                    ...
                    <div className="test-results-table">
                      <table>
                        ...
                        <tbody>
                          {codingSubmission.testResults.map((test, index) => (
                            <tr key={index}>
                              <td>Test {index + 1}</td>
                              <td className="test-input">{test.input}</td>
                              <td className="test-expected">{test.expected}</td>
                              <td className="test-actual">{test.actual}</td>
                              <td>
                                <span
