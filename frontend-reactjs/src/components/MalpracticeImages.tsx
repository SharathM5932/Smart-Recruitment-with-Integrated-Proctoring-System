...
const fetchMalpracticeData = async (id) => {
  const response = await fetch("http://localhost:3000/dashboard");
  if (!response.ok) {
    throw new Error("Network response was not ok");
  }
  const data = await response.json();
  const candidate = data.data.find((candidate) => candidate.id === id);
  return candidate;
};
...
const MalpracticeImages = () => {
  ...
  useEffect(() => {
    const loadMalpracticeData = async () => {
      try {
        setIsLoading(true);
        const candidateData = await fetchMalpracticeData(id);
    ...
    return (
      <div className="malpractice-loading">
        <div className="loading-spinner">
          ...
          <span>Loading proctoring images...</span>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="malpractice-error">
        <div className="error-content">
          <h3>Error loading proctoring images</h3>
          <p>{error.message}</p>
      ...
      {malpracticeImages.length === 0 ? (
        ...
        <div className="images-content">
          ...
          <div className="images-grid">
            {malpracticeImages.map((malpractice, index) => (
